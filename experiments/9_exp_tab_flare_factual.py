...
from teacher.explanation._counterfactual import _mixed_distance
...
def k_closest(instance, dataset, cont_idx, disc_idx, mad, k=5):
    order = np.argsort([_mixed_distance(instance, x, cont_idx, disc_idx, mad) for x in dataset])[1:k+1]
    return dataset[order]


def k_fidelity(instance, X_test, cont_idx, disc_idx, mad, dataset, neighborhood, explainer, bb, k=5):
    closest = k_closest(instance, X_test, cont_idx, disc_idx, mad, k)
    decoded_closest = [decode_instance(instance, dataset, neighborhood.get_fuzzy_variables()) for instance in closest]
    a = explainer.local_explainer.predict(np.array(decoded_closest))
    b = bb.predict(closest)
    return (a == b).sum() / len(b)
...
def main(ds, black_box, random_state, max_depth, size, cf_dist, min_num_examples, idx_record2explain, neighrange):
    ...
    path_results = 'factual-neighrange-search-v2\\'
    filename_results = path_results + 'f_performance_%s_%s_%s_%s_flore.csv' % (ds, black_box, idx_record2explain, neighrange)
    # %%
    ...
    results = f"{explainer.fidelity}, {int(explainer.hit())}, {k_fidelity(instance, X_test, cont_idx, disc_idx, mad, dataset, neighborhood, explainer, bb, k=3)}, {k_fidelity(instance, X_test, cont_idx, disc_idx, mad, dataset, neighborhood, explainer, bb, k=5)}, {k_fidelity(instance, X_test, cont_idx, disc_idx, mad, dataset, neighborhood, explainer, bb, k=7)}, {perf}\n"
    # print(results)
    # if not explainer.hit():
    #     logging.warning(idx_record2explain)
    #     logging.warning(explainer.exp_value)
    #     logging.warning(explainer.target)
    #     logging.warning(explainer.fidelity)
    with open(filename_results, 'a+') as f:
        # The results are fidelity, hit, k_fidelity with k=3, k_fidelity with k=5, k_fidelity with k=7 and perf
        f.write(results)
    # %%
