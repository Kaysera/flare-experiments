...
def distance_mh(x, cf_list, continuous_features, categorical_features, X, ratio_cont=None, agg=None):
    ...
    if ratio_cont is None:
        ...
        ratio_categorical = 1.0 - ratio_cont
    if ratio_continuous:
        dist += dist_cont * ratio_continuous
    if ratio_categorical:
        dist += dist_cate * ratio_categorical
    return dist
...
def plausibility_fixed(x, bb, cf_list, X_test, y_pred, continuous_features_all,
    ...
    for cf in cf_list:
        ...
        neigh_dist = [distance_mh(cf.reshape(1, -1), Xty.reshape(1, -1), continuous_features_all,
                        categorical_features_all, X_train, ratio_cont) for Xty in X_test_y]
        d = np.min(neigh_dist)
        sum_dist += d
    return sum_dist
...
def evaluate_cf_list(cf_list, x, bb, y_val, max_nbr_cf, variable_features, continuous_features_all,
    ...
    if nbr_cf_ > 0:
        ...
        plausibility_sum = plausibility(x, bb, cf_list, X_test, y_pred, continuous_features_all,
                                        categorical_features_all, X_train, ratio_cont)
        plausibility_fixed_ = plausibility_fixed(x, bb, cf_list, X_test, y_pred, continuous_features_all,
                                        categorical_features_all, X_train, ratio_cont)
        ...
        res = {
            ...
            'plausibility_fixed': plausibility_fixed_,
